"""GAP-78: test per /api/db/messages/recent, /api/messages/next e /api/draft."""
...
def test_messages_recent_always_200():
    r = _app_client().get("/api/db/messages/recent")
    assert r.status_code == 200


def test_messages_recent_has_required_keys():
    data = _app_client().get("/api/db/messages/recent").get_json()
...
def test_messages_recent_limit_param_accepted():
    r = _app_client().get("/api/db/messages/recent?limit=5")
    assert r.status_code == 200
